...
config_file = 'configs/aoi/connector/yolox_s_8x8_300e_coco_connector_with_pin_box.py'
# 从 model zoo 下载 checkpoint 并放在 `checkpoints/` 文件下
# 网址为: http://download.openmmlab.com/mmdetection/v2.0/faster_rcnn/faster_rcnn_r50_fpn_1x_coco/faster_rcnn_r50_fpn_1x_coco_20200130-047c8118.pth
# checkpoint_file = 'work_dirs/faster_rcnn_r50_fpn_2x_coco_mirror_surface/latest.pth'
checkpoint_file = 'work_dirs/yolox_s_8x8_300e_coco_connector_with_pin_box/best_bbox_mAP_epoch_337.pth'
device = 'cuda'
# 初始化检测器
...
for item in test_filelist:
    ...
    for index,finger in enumerate(res[0]):
        # print(finger)
        if finger[4]>0.7:
            dst=cv2.rectangle(src,(int(finger[0]),int(finger[1])),(int(finger[2]),int(finger[3])),(0,255,0),2)
    
    for index,finger in enumerate(res[1]):
        # print(finger)
        if finger[4]>0.7:
            dst=cv2.rectangle(dst,(int(finger[0]),int(finger[1])),(int(finger[2]),int(finger[3])),(0,255,0),2)
    cv2.imwrite(r'/home/E/data/aoi/Connector/connector_detection_ds/test_res_finger_and_pin/'+item+"_.jpg",dst)
