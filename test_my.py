...
config_file = 'configs/aoi/mirror_surface/my_mask_rcnn_mirror_surface.py'
# 从 model zoo 下载 checkpoint 并放在 `checkpoints/` 文件下
# 网址为: http://download.openmmlab.com/mmdetection/v2.0/faster_rcnn/faster_rcnn_r50_fpn_1x_coco/faster_rcnn_r50_fpn_1x_coco_20200130-047c8118.pth
checkpoint_file = 'work_dirs/my_mask_rcnn_mirror_surface/latest.pth'
device = 'cuda'
# 初始化检测器
model = init_detector(config_file, checkpoint_file, device=device)
# 推理演示图像
...
for item in test_filelist:
    ...
    model.show_result(test_dir+item,res,out_file="resImg/my_mask_rcnn_mirror_surface/"+item)
    time_end=time.time()
